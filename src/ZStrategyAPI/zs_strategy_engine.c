    ...
    uint32_t evtype, zs_data_head_t* evdata)
{
    ...
    {
        strategy = *(zs_cta_strategy_t**)ztl_array_at(zse->AllStrategy, i);
        if (!strategy) {
            continue;
        }
...
int zs_strategy_engine_load(zs_strategy_engine_t* zse, ztl_array_t* libpaths)
{
    ...
    if (libpaths)
    {
        ...
        {
            libpath = *(const char**)ztl_array_at(libpaths, i);
    ...
    void** dst = ztl_array_push(zse->StrategyEntries);
    *dst = strategy_entry;
#endif
    ...
    const char* strategy_name, const char* setting)
{
    ...
    strategy_entry = ztl_array_find(zse->StrategyEntries, (char*)strategy_name, _strategy_entry_name_comp);
    if (!strategy_entry)
    {
    ...
    ZSExchangeID exchangeid, const char* symbol)
{
    ...
    dictAdd(zse->Tick2StrategyList, (void*)sid, strategy);
