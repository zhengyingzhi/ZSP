...
void* stg_demo_create(const char* str, int reserve)
{
...
void stg_demo_on_order(void* stg, zs_algorithm_t* context, zs_order_t* order)
{
    printf("stg demo got order: %s,%d,%d,%.2f\n", order->Symbol, order->Direction, 
        order->Quantity, order->Price);
}

void stg_demo_on_trade(void* stg, zs_algorithm_t* context, zs_trade_t* trade)
{
...
// 行情通知
void stg_demo_on_bardata(void* stg, zs_algorithm_t* context, zs_bar_reader_t* barReader)
{
...
void stg_demo_on_tickdata(void* stg, zs_algorithm_t* context, zs_tick_t* tickData)
{
    // visit the tick data
}
...
int zs_strategy_entry(zs_strategy_api_t** ppStragegy)
{
    *ppStragegy = &stg_demo;
    return 0;
}
